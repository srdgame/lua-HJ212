...
function LA:load_from_db()
	...
	sample_list, err = self._db:read_samples(hour_start_time, self._start)
	if sample_list then
...
function LA:_calc_sample(list, start, etime)
	...
	for _, v in ipairs(list) do
		if helper.flag_can_calc(v.flag) then
			...
			-- self:log('debug', 'LA.calc_sample', val_cou, v.cou or val, val_min, val_max)
		end
	end

	local val_avg = val_t > 0 and 10 * math.log((val_cou / val_t), 10) or 0

	--self:log('debug', 'LA.calc_sample', #list, val_cou, val_avg, val_min, val_max)
...
function LA:on_min_trigger(now, duration)
	...
	while start < (now - duration) do
		...
		if self._min_list:find(etime) then
			...
			local val = self:_calc_sample(list, start, etime)
	...
	local val = self:_calc_sample(list, start, now)
...
function LA:_calc_cou(list, start, etime)
	local flag = #list == 0 and types.FLAG.Connection or nil
...
function LA:on_hour_trigger(now, duration)
	...
	while start < (now - duration) do
		...
		if self._hour_list:find(etime) then
			...
			local val = self:_calc_cou(list, start, etime)
	...
	local val = self:_calc_cou(list, start, now)
